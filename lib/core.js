...
const SELF_RELEASE_PREFIX = 'releases/dsh-cloud-sync'
const SELF_RELEASE_LATEST = `${SELF_RELEASE_PREFIX}/latest.json`
...
function selfReleaseKey(version) { return `${SELF_RELEASE_PREFIX}/${version}.tgz` }

function parseSelfRelease(value) {
  if (value === null || typeof value !== 'object' || value.packageName !== SELF_PACKAGE || typeof value.version !== 'string' || typeof value.sha256 !== 'string' || !/^[a-f0-9]{64}$/i.test(value.sha256) || value.objectKey !== selfReleaseKey(value.version)) throw new Error('Remote Cloud Sync release metadata is invalid')
  releaseVersionParts(value.version) ?? (() => { throw new Error('Remote Cloud Sync release version is invalid') })()
  return { packageName: SELF_PACKAGE, version: value.version, objectKey: value.objectKey, sha256: value.sha256.toLowerCase(), createdAt: typeof value.createdAt === 'string' ? value.createdAt : undefined }
}

async function latestSelfRelease(provider) {
  try { return parseSelfRelease(JSON.parse((await provider.get(SELF_RELEASE_LATEST)).toString('utf8'))) } catch (error) {
...
async function publishSelfRelease(provider, home) {
  const version = await ownVersion()
  const manifest = await readJson(join(profileDir('web', home), 'package.json'), {})
  const archivePath = configuredSelfArchive(manifest.dependencies?.[SELF_PACKAGE], profileDir('web', home))
  if (archivePath === undefined || !existsSync(archivePath)) return { published: false, reason: 'local-package-not-found', version }
  const remote = await latestSelfRelease(provider)
  if (remote !== undefined && compareVersions(remote.version, version) > 0) return { published: false, reason: 'remote-is-newer', version, remoteVersion: remote.version }
  const archive = await readFile(archivePath)
  const release = { packageName: SELF_PACKAGE, version, objectKey: selfReleaseKey(version), sha256: hash(archive), createdAt: new Date().toISOString() }
  await provider.put(release.objectKey, archive)
  await provider.put(SELF_RELEASE_LATEST, Buffer.from(`${JSON.stringify(release, null, 2)}\n`))
  return { published: true, ...release }
}

export async function checkSelfUpdate({ home = dshHome() } = {}) {
  const settings = await loadSettings(home)
  const localVersion = await ownVersion()
  if (settings.provider?.type === 'none' || (settings.provider?.type === 'webdav' && settings.provider.url === '')) return { configured: false, localVersion, available: false }
  const release = await latestSelfRelease(providerFrom(settings, home))
  if (release === undefined) return { configured: true, localVersion, available: false }
  ...
  return { configured: true, localVersion, localArchiveSha256, available: versionComparison > 0 || sameVersionRevision, sameVersionRevision, release }
}
...
export async function synchronizeSnapshots({ home = dshHome(), strategy, resolveConflicts, selectedItems } = {}) {
  ...
  if (cloud === undefined || policy === 'local') {
    ...
    const release = await publishSelfRelease(provider, home)
    return { strategy: policy, direction: 'uploaded', pnpmShims, missingLocalSources, release, history, createdAt: local.snapshot.createdAt, profiles: local.snapshot.profiles.map(profile => profile.name), sources: local.snapshot.sources.map(source => source.name), sha256: local.sha256, byteLength: local.byteLength }
  }
  ...
  const release = await publishSelfRelease(provider, home)
  return { strategy: policy, direction: 'merged', pnpmShims, missingLocalSources, release, history, ...result }
}
...
export async function updateSelf({ home = dshHome() } = {}) {
  const update = await checkSelfUpdate({ home })
  if (!update.configured) throw new Error('Configure WebDAV before checking for Cloud Sync updates')
  if (!update.available || update.release === undefined) return { ...update, updated: false }
  const provider = providerFrom(await loadSettings(home), home)
  const archive = await provider.get(update.release.objectKey)
  if (hash(archive) !== update.release.sha256) throw new Error('Cloud Sync update checksum verification failed')
  const output = join(syncRoot(home), 'releases', basename(update.release.objectKey))
  await mkdir(dirname(output), { recursive: true })
