...
const SAVED_PROVIDER_TYPES = ['webdav', 's3', 'oss', 'cos', 'minio']
...
function normalizeSavedProviders(settings) {
  ...
  if (SAVED_PROVIDER_TYPES.includes(active?.type)) {
    ...
    if (profile !== undefined && (profile.url !== '' || profile.endpoint !== '' || profile.bucket !== '')) savedProviders[active.type] = profile
  }
...
export async function loadSettings(home = dshHome()) {
  ...
  if (type !== 'webdav' && !isObjectStorageType(type)) return { ...defaultSettings, ...settings, savedProviders: normalizeSavedProviders(settings) }
