...
async function saveSecret(secret, home) {
  if (secret === '') { await rm(credentialsPath(home), { force: true }); return }
  const payload = process.platform === 'win32' ? { scheme: 'dpapi', secret: await dpapi(secret, 'protect') } : { scheme: 'file-0600', secret }
...
async function persistSettings(settings, home) {
  ...
  await saveSecret(secret, home)
  const stored = { ...settings, provider: maskedProvider(settings.provider, secret) }
...
export async function loadSettings(home = dshHome()) {
  ...
  if (type !== 'webdav' && !isObjectStorageType(type) && type !== 'none') {
  ...
  if (type !== 'webdav' && !isObjectStorageType(type)) return settings
  const field = type === 'webdav' ? 'password' : 'secretAccessKey'
  if (settings.provider[field] !== '' && settings.provider[field] !== '<stored-in-credentials>') {
    ...
    return { ...settings, provider: { ...settings.provider, [field]: secret } }
  }
  return { ...defaultSettings, ...settings, autoSync: { ...defaultSettings.autoSync, ...(settings.autoSync ?? {}) }, syncScope: { ...defaultSettings.syncScope, ...(settings.syncScope ?? {}) }, encryption: { ...defaultSettings.encryption, ...(settings.encryption ?? {}) }, provider: { ...settings.provider, [field]: settings.provider[field] === '' ? '' : await readSecret(home) } }
}
...
async function normalizeObjectStorageSettings(settings, home) {
  ...
  const secretAccessKey = settings.provider.secretAccessKey === '' && previous.provider?.type === type ? previous.provider.secretAccessKey ?? '' : settings.provider.secretAccessKey
...
class S3Provider {
  ...
  async request(method, path, { query = [], body, ifMatch, ifNoneMatch } = {}) {
    ...
    if (ifNoneMatch === true && this.type !== 'oss') headers['if-none-match'] = '*'
