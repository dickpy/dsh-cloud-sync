window.__ModuleLoader__.load({ id: '@dsh-local/dsh-cloud-sync', factory: (require) => {
  ...
  function DiffDialog({ changes, selectedItems, setSelectedItems, onClose, onSync, busy }) {
    ...
    return h('div', { className: 'modalBack', onMouseDown: onClose }, h('section', { className: 'modal diffModal', role: 'dialog', 'aria-modal': 'true', 'aria-label': '同步差异', onMouseDown: event => event.stopPropagation() },
      h('div', { className: 'diffHead' }, h('div', { className: 'modalHead' }, h('div', null, h('h3', null, '选择要同步的变化'), h('p', { className: 'hint' }, '勾选的项目会以当前设备的版本参与本次智能合并。未勾选项目保持云端版本。')), h('button', { className: 'modalClose', onClick: onClose, 'aria-label': '关闭' }, '×')), h('div', { className: 'actions' }, h('button', { disabled: busy !== '', onClick: () => selectAll(true) }, '全选'), h('button', { disabled: busy !== '', onClick: () => selectAll(false) }, '全不选'))),
      h('div', { className: 'diffBody' }, h('div', { className: 'diffGrid head' }, h('span', null), h('span', null, '类别'), h('span', null, '项目'), h('span', null, '本地'), h('span', null, '云端')), changes.map(item => h('div', { className: 'diffGrid', key: item.id }, h('span', { className: 'diffCheck' }, h('input', { type: 'checkbox', checked: selected.has(item.id), onChange: () => toggle(item) })), h('span', { className: 'diffKind' }, item.kind === 'plugin' ? '插件' : '配置'), h('span', null, `${item.profile} / ${item.name}`), h('span', null, item.local || '未安装'), h('span', null, item.remote || '未安装')))),
      h('div', { className: 'diffFooter' }, h('span', { className: 'muted' }, `已选择 ${selectedItems.length} / ${changes.length} 项`), h('div', { className: 'serviceActions' }, h('button', { disabled: busy !== '', onClick: onClose }, '取消'), h('button', { className: 'primary', disabled: busy !== '' || selectedItems.length === 0, onClick: onSync }, '同步选中项')))
    ))
  }
  ...
  function ConfigurationTab({ run, busy, setResult }) {
    ...
    useEffect(() => { if (webdavConfigured) { setDeviceName(settings.deviceName || ''); setIntervalMinutes(settings.autoSync?.intervalMinutes || 30); refreshChanges().catch(() => {}); refreshHistory().catch(() => {}) } }, [webdavConfigured])
    ...
    return h(React.Fragment, null,
      ...
      configured ? h('section', { className: 'group' }, h('div', { className: 'nextStep' },
        h('h3', null, changes.length ? `发现 ${changes.length} 项待处理变化` : '准备同步'),
        h('p', { className: 'hint' }, changes.length ? '先在对比窗口选择需要同步的插件或配置，再开始同步。' : '检查差异后，可以选择部分项目同步；一键同步会合并全部变化。'),
        h('div', { className: 'actions' }, h('button', { disabled: busy !== '', onClick: run('diff', async () => { const value = await refreshChanges(); setDiffOpen(true); return `发现 ${value.items.length} 项差异。` }) }, '检查差异'), changes.length ? h('button', { disabled: busy !== '', onClick: () => setDiffOpen(true) }, `查看 ${changes.length} 项差异`) : null, h('button', { className: 'primary', disabled: busy !== '', onClick: sync(undefined, true) }, busy === 'sync' ? '正在同步' : '一键同步全部')),
      ...
      configured ? h('details', null, h('summary', null, '同步方式（可选）'), h('div', null,
        h('p', { className: 'hint' }, '智能合并适合日常使用。云端优先会恢复云端配置，本地优先会覆盖云端快照。'),
        h(Field, { label: '本次同步方式', wide: true }, h('select', { value: strategy, onChange: event => { setStrategy(event.target.value); setConflicts([]) } }, h('option', { value: 'smart' }, '智能合并（推荐）'), h('option', { value: 'cloud' }, '云端优先'), h('option', { value: 'local' }, '本地优先')))
      )) : null,
      configured ? h('details', null, h('summary', null, '自动化与安全（可选）'), h('div', null,
        h('p', { className: 'hint' }, '设备名称只用于历史记录。自动同步仅在检测到变化时运行；它不会擅自处理冲突。客户端加密需要所有设备输入同一口令。'),
        h('div', { className: 'grid' }, h(Field, { label: '设备名称' }, h('input', { value: deviceName, onChange: event => setDeviceName(event.target.value), placeholder: '例如：办公室电脑' })), h(Field, { label: '检查间隔（分钟）' }, h('input', { type: 'number', min: 5, max: 1440, value: intervalMinutes, onChange: event => setIntervalMinutes(event.target.value) }))),
      ...
      diffOpen ? h(DiffDialog, { changes, selectedItems, setSelectedItems, busy, onClose: () => setDiffOpen(false), onSync: () => { setDiffOpen(false); sync()() } }) : null
    )
    return h(React.Fragment, null,
      service({ title: 'WebDAV', active: webdavConfigured, detail: webdavDetail, edit: () => setEditingWebDav(true) }),
