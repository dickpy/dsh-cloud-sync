import { adoptConfiguredPlugin, backupLocalPlugin, checkSelfUpdate, clearSyncProvider, connectProvider, connectWebDav, getGithubDeviceAuthorizationStatus, getPublicSettings, getSyncInventory, installConfiguredPlugin, listSnapshotHistory, lockEncryption, pollGithubDeviceAuthorization, previewSyncChanges, pullSnapshot, runAutomaticSync, startGithubDeviceAuthorization, status, synchronizeSnapshots, unlockEncryption, uninstallPlugin, updateSelf, updateSyncPreferences } from './core.js'
...
export function apply(ctx) {
  const routes = [
    ...
    route('/api/dsh-cloud-sync/github/device/status', () => getGithubDeviceAuthorizationStatus()),
    route('/api/dsh-cloud-sync/github/device/start', () => startGithubDeviceAuthorization()),
    route('/api/dsh-cloud-sync/github/device/poll', input => pollGithubDeviceAuthorization(input)),
