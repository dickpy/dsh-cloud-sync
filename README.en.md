...
`@dsh-local/dsh-cloud-sync` · WebDAV · AES-256-GCM client-side encryption · Snapshot history & rollback
...
**DSH Cloud Sync** is a [DeepSeek Harness](https://github.com/deepseek-ai/dsh) (DSH) bundle for portable profile recovery. It synchronizes small, reproducible profile files to WebDAV rather than copying `node_modules`, then lets DSH/pnpm rebuild packages on the target computer.
...
- Checks the latest [GitHub Release](https://github.com/dickpy/dsh-cloud-sync/releases) when the settings page opens; no WebDAV connection required;
...
- Use a private HTTPS WebDAV directory and an app password; plain HTTP Basic auth is rejected;
...
- On Windows the remembered app password is protected with DPAPI for the current user; on other platforms it is stored in a separate owner-only (`0600`) local credentials file; `settings.json` never contains the password;
...
# Unit tests (built-in mock WebDAV server)
pnpm test
```
...
lib/
  ...
  core.js    # Core logic: WebDAV client, snapshots, encryption, plugin lifecycle
  client.js  # Web panel (React, injected into the Settings "Cloud Sync" section)
test/
  core.test.mjs  # Integration tests with a mock WebDAV server
...
**Q: Why WebDAV only?**
A: The current release focuses on WebDAV (works well with Nutstore and similar services). The endpoint must use HTTPS.
